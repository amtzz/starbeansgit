...
@PageTitle("Sale")
@Route(value = "stock/outbound", layout = MainLayout.class)
public class OutboundStockView extends Div {
...
//                () -> Notification.show("Store not found!!"));
    ...
    private Component createCheckoutForm() {
        ...
        checkoutForm.add(createOutboundProductSection());
...
//        checkoutForm.add(new Hr());
    ...
    private Section createOutboundProductSection() {
    ...
    private void addProductAction(String sku, String amountValue) {
        ...
        if (saleItems.containsKey(sku)) {
            final StockItem item = saleItems.get(sku);
    ...
    private Footer createAsideFooter() {
        ...
        Button cancel = new Button("Cancel Sale");
        cancel.addThemeVariants(ButtonVariant.LUMO_TERTIARY);
        cancel.addClickListener(buttonClickEvent -> clearList());

        Button saveArrivalButton = new Button("Finish Sale", new Icon(VaadinIcon.SIGN_OUT));
        ...
        saveArrivalButton.addClickListener(buttonClickEvent -> doSaleAction());
    ...
    private void doSaleAction() {
        try {
            for (StockItem saleItem : saleItems.values()) {
                stockItemService.sellProduct(saleItem);
            }
            Notification.show("Successful Sale! Printing Receipt.");
            clearList();
        } catch (Exception e) {
            showError("Error while processing sale: " + e.getMessage());
        }
    }
    ...
    private void refreshSaleList() {
        ...
        for (StockItem item : saleItems.values()) {
